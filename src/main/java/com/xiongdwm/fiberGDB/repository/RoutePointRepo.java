...
public interface RoutePointRepo extends ReactiveNeo4jRepository<RoutePoint, Long> {
   ...
   Flux<RoutePointDTOProjection> findResultByCypher(@Param("startId")Long startId, @Param("endId") Long endId, @Param("weightLimit") double weightLimit);
    ...
    @Query("MATCH p = (start:RoutePoint {id: $startId})-[fiber:FIBER*]-(end:RoutePoint {id: $endId}) " +
            ...
            "REDUCE (sum=0, rel IN relationships(p) | sum + rel.weight) <= $weightLimit "+
            "RETURN nodes(p) AS routes")
    List<Result> findResultByCypherV2(@Param("startId")Long startId, @Param("endId") Long endId, @Param("weightLimit") double weightLimit);
